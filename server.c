...
// Struktura TLV do komunikacji klient-serwer
struct TLV
{
    uint8_t type;
    uint8_t length;
...
struct GameState
{
    char word[WORD_LENGTH]; // Wylosowane słowo
    char guessed[WORD_LENGTH];  // Aktualny stan słowa
    int attempts_left;  // Pozostałe próby
};
...
int main()
{
    ...
    // Oczekiwanie na połączenia klientów
...
// Funkcja tworząca proces demona
void daemonize()
{
    pid_t pid = fork();
...
// Wątek multicastowy ogłaszający serwer w sieci
void *multicast_announcer(void *arg)
{
    int sock;
